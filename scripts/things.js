...
he3d.game.things.spawnMissile=function(thing,missile_type){
	...
	pthing.delta[0]=pthing.delta[0]*pthing.basething.speed*ANIM_FPS;	// Was 25? What is 25?
	pthing.delta[1]=pthing.delta[1]*pthing.basething.speed*ANIM_FPS;  	//pthing.basething.speed was 25
...
he3d.game.things.damageThing=function(target,source,attacker,damage){
	...
	if(source&&!(target.flags & mobjflag_t.MF_NOCLIP)&&
		...
		ang = this.thing_to_thing_angle(source.x, source.y,
			target.x, target.y, 0);
		...
		if (damage < 40
			...
			&& target.pos[3] - source.pos[3] > 64 * 65535
			&& (he3d.game.random() & 1))
		{
			delta[0]*=-1;
			delta[2]*=-1;
	...
	if(target.health<=0&&!target.player){
		...
		if(target.painstate
			&&target.painstate!=target.curstate){
			...
			target.sprite=he3d.game.things.getSpriteName(
									target.painstate,spdir);

			if(target.sprite
				&&!(target.flags&mobjflag_t.MF_NOGRAVITY))
				target.pos[1]=target.sector.floor
								+(target.sprite.height/2);
		}
	}
}
...
he3d.game.things.killThing=function(killer, target){
	...
	if(target.health < -target.spawnhealth
		&&target.xdeathstate)
...
he3d.game.things.checkThing=function(thing, otherthing){
	...
	// thing cant be hit
	if(!(thing.flags &
			(mobjflag_t.MF_SOLID|mobjflag_t.MF_SPECIAL|
				mobjflag_t.MF_SHOOTABLE)))
		return true;
	...
	if (otherthing.flags & mobjflag_t.MF_MISSILE)
	{
		...
		if (otherthing.damage)
		{
			...
			he3d.game.things.damageThing(thing,otherthing,otherthing,damage);
		}
...
he3d.game.things.update=function(){
	...
	for(var t=0;t<he3d.game.map.things.length;t++){
		...
		if((thing.curstate==statenum_t.S_NULL||!state_t[thing.curstate])
			&&!(thing.flags & mobjflag_t.MF_MISSILE)
			&&!thing.player)
			continue;

		if((thing.flags & mobjflag_t.MF_MISSILE)
			&&t==he3d.game.map.things.length-1
			&&thing.curstate==statenum_t.S_NULL){
				
			he3d.game.map.things.pop();
			continue;
		}
		...
		if(!thing.player&&thing.health>0){
			if(thing.delta[0]||thing.delta[1]||thing.delta[2]){
				var newpos=[
				...
				if(!he3d.game.map.checkMoveLocation(thing,Math.round(newpos[0]),Math.round(newpos[2]))
			...
			switch(state_t[thing.curstate].action){
				...
				case "A_Chase":
					...
					if(!thing.target){
						...
						view_angle=he3d.game.things.thing_to_thing_angle(
							thing.pos[0],thing.pos[2],
							thing.target.pos[0],thing.target.pos[2],0);
						view_angle=180-pangle+view_angle+25;
						if(view_angle<0)
							view_angle+=360;
				...
				case "A_BruisAttack":	//Check range and switch between melee or missile
					...
					if(thing.tic==0){
						if(tdist<=64)
							he3d.game.things.damageThing(thing.target,thing,thing,(he3d.game.random()%8+1)*10);
						else
							he3d.game.things.spawnMissile(thing,mobjtype_t.MT_BRUISERSHOT);
					}
				case "A_TroopAttack":	//Check range and switch between melee or missile
					...
					if(thing.tic==0){
						if(tdist<=64)
							...
							he3d.game.things.spawnMissile(thing,mobjtype_t.MT_TROOPSHOT);
					}
					break;
				case "A_HeadAttack":	//Check range and switch between melee or missile
					//If melee do damage (P_Random()%6+1)*10 to target
					//he3d.game.things.damageThing(thing.target,thing,thing,he3d.game.(random()%6+1)*10)
					
					if(thing.tic==0){
						if(tdist<=64)
							he3d.game.things.damageThing(thing.target,thing,thing,(he3d.game.random()%6+1)*10);
						else
							he3d.game.things.spawnMissile(thing,mobjtype_t.MT_HEADSHOT);
					}
					break;
				case "A_CyberAttack":
					if(thing.tic==0){
						he3d.game.things.spawnMissile(thing,mobjtype_t.MT_ROCKET);
					}
					break;
				case "A_SkelMissile":
					if(thing.tic==0){
						he3d.game.things.spawnMissile(thing,mobjtype_t.MT_TRACER);
					}
					break;
				case "A_SkelFist":	//Check range for melee
					// damage (P_Random()%10+1)*6
					//he3d.game.things.damageThing(thing.target,thing,thing,he3d.game.(random()%10+1)*6)
					if(thing.tic==0){
						if(tdist<=64)
							he3d.game.things.damageThing(thing.target,thing,thing,(he3d.game.random()%10+1)*6);	
					}				
				...
				case "A_SargAttack":	//Check range for melee				
					...
					//he3d.game.things.damageThing(thing.target,thing,thing,he3d.game.(random()%10+1)*4)
					if(thing.tic==0){
						if(tdist<=64)
							he3d.game.things.damageThing(
									thing.target,thing,
									thing,(he3d.game.random()%10+1)*4);	
					}
				case "A_SPosAttack":
				case "A_PosAttack":
					thing.flash=true;
...
he3d.game.things.buildSprites=function(){
	...
	for(var s in he3d.game.map.sprites){
		if(!he3d.game.map.sprites)
			...
			data=he3d.tools.interleaveFloat32Arrays([3,3],[verts,texcoords]);
